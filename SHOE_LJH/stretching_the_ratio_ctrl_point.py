...
# --- 전역 변수 설정 (파일 상단 또는 __main__ 위에 추가) ---
CORRECT_SHOE_SIZE = 250
TARGET_SHOE_SIZE = 270
ratio = CORRECT_SHOE_SIZE /TARGET_SHOE_SIZE  # 1.08
# --------------------------------------------------------
...
def run_open_fit(image_path, n_contour_points=200, n_ctrl_points=25):
    ...
    # ★★★ 핵심 변경 부분: 제어점 확대 ★★★
    print(f"4-1. Scaling control points by ratio: {ratio:.3f}")
    scaled_ctrl_points = initial_ctrl_points * ratio

    print("5. Launching interactive editor (mm mode)...")
    # 확대된 제어점을 DraggableCtrl에 전달합니다.
    app = DraggableCtrl(
        scaled_ctrl_points, # <--- scaled_ctrl_points 사용
        background_contour=contour_mm, # 원본 배경 윤곽선은 그대로 유지하여 비교 가능
        save_stem=save_stem,
        title_suffix=f" (Scaled x{ratio:.2f} for {TARGET_SHOE_SIZE}mm)"
    )
    app.ax.set_title(f"Drag Control Points to Edit the Spline (Open) — Scaled to ≈{TARGET_SHOE_SIZE} mm")
    plt.show()
...
if __name__ == "__main__":
    ...
    IMAGE_FILE = "output_outlines_1030\L\irish_250L.jpg"
