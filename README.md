...
**향후 과제 (Future Work)**
* **강건한 모델을 위한 데이터 증강:** 회전(Rotation), 비틀림(Shearing), 시점 변환(Perspective Transform) 등의 Data Augmentation 기법을 적용하여, 다양한 촬영 환경에서도 모델이 강건하게(Robust) 동작하도록 일반화 성능을 높일 계획입니다.
