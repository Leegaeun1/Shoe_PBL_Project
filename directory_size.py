...
PER_SIZE_OUTDIR  = f"."
os.makedirs(PER_SIZE_OUTDIR, exist_ok=True)
...
class DraggableCtrl:
    ...
    def save_current(self):
        # 현재 ctrl은 offset(첫 점 원점기준) 상태임 -> 실제 CSV 저장은 save_ctrl_to_csv가 내부에서 정규화 다시 하므로 그대로 전달
        save_ctrl_to_sizebucket(self.image_path, self.ctrl)
...
def _ensure_master_header(path, n_ctrl, sep=", "):
    """마스터 CSV에 헤더가 없으면 생성: size,x1,y1,...,xN,yN"""
...
#def _ensure_header_for_file(path, n_ctrl, sep=", "):
    """파일에 헤더 없으면 생성: size,x1,y1,...,xN,yN"""
    if os.path.exists(path) and os.path.getsize(path) > 0:
        return
    cols = ["size"]
    for i in range(1, n_ctrl + 1):
...
def save_ctrl_to_sizebucket(image_path, ctrl_points, out_dir=PER_SIZE_OUTDIR, sep=", "):
    """
    같은 사이즈는 같은 파일로 모아 저장합니다.
      - 230 → <out_dir>/230_train.csv
      - 250 → <out_dir>/250_train.csv
    좌상단 원점 정렬은 기존 규칙 유지.
    """
    n_ctrl = ctrl_points.shape[0]
    size_mm = parse_size_mm_from_filename(image_path, fallback=250.0)
    size_tag = int(round(size_mm))

    # 파일 경로 결정
    csv_path = os.path.join(out_dir, f"train_{size_tag}.csv")
    #_ensure_header_for_file(csv_path, n_ctrl, sep=sep)

    coords = ctrl_points.copy()
    # 좌상단(최솟값) 기준 정렬 — 기존 저장 규칙 유지
    min_x, min_y = coords.min(axis=0)
    coords[:, 0] -= min_x
    coords[:, 1] -= min_y

    flat = coords.flatten()
    line = sep.join([f"{size_tag:d}"] + [f"{v:.6f}" for v in flat])

    with open(csv_path, "a", encoding="utf-8") as f:
        f.write(line + "\n")

    print(f"[APPEND] -> {csv_path}  (size={size_tag}, ctrl_pts={n_ctrl})")
...
def run_open_fit_DIR(DIR_path, n_contour_points=200, n_ctrl_points=25):
    ...
    for img in jpgs:
        try:
            ...
            # (2) 즉시 저장
            save_ctrl_to_sizebucket(img, ctrl)
...
if __name__ == "__main__":
    ...
    DIR_PATH = "./Adidas - Runfalcon 5 Wide"  # <- 여기에 대상 디렉토리 경로 입력
