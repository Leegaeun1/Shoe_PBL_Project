...
#FILE_B = fr"Fin_Excel_Data1\pred_Data_230_280_{OUTPUT_NAME}_{TYPE}.csv"
FILE_B = "Fin_Excel_Data_CTRL80\control_points_scaled_230_280mm.csv"
...
TYPE_COLS = {
    'type': 0, # 'type' 컬럼 인덱스
    'size': 2 # 'size' 컬럼 인덱스 (공백 포함 주의)
}


def load_rows_by_type(path, target_type, type_col_idx, size_col_idx):
    """
    지정된 target_type과 일치하는 행만 불러와 size를 키로 하는 딕셔너리를 반환합니다.
    """
    out = {}
    with open(path, "r", encoding="utf-8") as f:
        r = csv.reader(f)
        header = [c.strip() for c in next(r)] # 헤더 읽기
        
        # 헤더 인덱스 재확인 (혹시 모를 변경 대비)
        # try:
        #     type_col_idx = header.index('type')
        #     size_col_idx = header.index('size')
        # except ValueError:
        #     raise RuntimeError("CSV 헤더에서 'type' 또는 'size' 컬럼을 찾을 수 없습니다.")

        for i, row in enumerate(r):
            if not row or len(row) < size_col_idx + 3: # size와 최소한 x1, y1까지 있어야 함
                continue
            ...
            # 2. Size 및 좌표 추출
            try:
                # Size (인덱스 size_col_idx)
                size = int(float(row[size_col_idx].strip()))
                
                # 좌표 (size 이후부터 시작)
                # type, side, size (3개) 이후부터 좌표 시작이라고 가정
                # row[3:] 부터 수치형 좌표라고 가정합니다.
                coords_str = [t.strip() for t in row[3:] if t.strip()]
                nums = [float(t) for t in coords_str if _is_num(t)]
                
            except ValueError:
                # 타입은 맞으나 size나 좌표가 숫자가 아님 (스킵)
                continue
            
            if len(nums) < 2:
                continue

            xy = np.array(nums, float)
            if xy.size % 2 == 1:
                xy = xy[:-1]
            if xy.size < 4:
                continue
    ...
    if not out:
        raise RuntimeError(f"Type '{target_type}'에 해당하는 유효한 수치 행이 {path}에 없습니다.")
    return out
...
def main():
    ...
    # ------------------ 데이터 로드 수정 ------------------
    # FILE_A: Type이 이미 파일명에 있으므로, load_rows_by_type 사용 (TYPE 필터링 적용)
    print(f"[INFO] FILE_A 로딩 및 TYPE 필터링: {TYPE}")
    dataA = load_rows_by_type(FILE_A, TYPE, TYPE_COLS['type'], TYPE_COLS['size'])

    # FILE_B: 여러 TYPE이 섞여 있으므로, load_rows_by_type 사용 (TYPE 필터링 적용)
    print(f"[INFO] FILE_B 로딩 및 TYPE 필터링: {TYPE}")
    dataB = load_rows_by_type(FILE_B, TYPE, TYPE_COLS['type'], TYPE_COLS['size'])
    # ----------------------------------------------------
