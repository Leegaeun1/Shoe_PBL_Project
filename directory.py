...
MASTER_CSV = f"control_points_master_{currentTime}.csv"
...
class DraggableCtrl:
    ...
    def save_current(self):
        ...
        save_ctrl_to_master(self.image_path, self.ctrl)
...
def save_ctrl_to_master(image_path, ctrl_points, master_csv=MASTER_CSV, sep=", "):
    """
    하나의 마스터 CSV 파일에 행을 '추가'합니다.
    형식:
      size, x1,y1,x2,y2,...,xN,yN
    - 좌상단 원점 정렬(기존 로직 유지)
    - 값은 소수점 6자리 고정
    """
    ...
    flat = coords.flatten()
    line = sep.join([f"{size_mm:.0f}"] + [f"{v:.6f}" for v in flat])

    with open(master_csv, "a", encoding="utf-8") as f:
        f.write(line + "\n")

    print(f"[APPEND] -> {master_csv}  (size≈{size_mm:.0f}, {n_ctrl} ctrl pts)")
