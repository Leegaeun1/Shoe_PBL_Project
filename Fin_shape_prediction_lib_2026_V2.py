...
# -----------------------------------------------------
    ...
    def analyze_type_clusters(self, save_dir, threshold=None, n_clusters=4):
        ...
        # Ward 방식의 Linkage Matrix (Z) 계산
        # Z의 컬럼: [클러스터1, 클러스터2, 거리(Distance), 샘플개수]
        ...
        if n_clusters is not None:
            ...
            # n_clusters로 나뉘는 지점의 거리(Threshold) 역산
            # Z[-(k-1), 2]는 k개에서 k-1개로 합쳐질 때의 거리입니다.
            # 따라서 k개로 나뉘려면 이 거리보다는 작게 잘라야 합니다.
            if n_clusters > 1 and len(Z) >= n_clusters:
                # k개로 묶이기 직전의 병합 거리 (이 거리 이상이면 k-1개가 됨)
                cut_threshold = Z[-(n_clusters-1), 2]
        ...
        # Z(Linkage Matrix)와 threshold를 함께 넘김
        self._save_cluster_results(save_dir, valid_types, dist_matrix, cluster_ids, Z, cut_mode, cut_threshold)
        print(f"  [Result] {cut_mode} -> Created {len(set(cluster_ids))} Groups. (Cut Threshold: {cut_threshold:.4f})")

    def _save_cluster_results(self, save_dir, valid_types, dist_matrix, cluster_ids, Z, cut_mode, cut_threshold):
        # 1. 거리 행렬 저장
        ...
        # 3. Linkage Matrix (병합 과정 수치) 저장
        # 이 파일은 덴드로그램을 그리는 원천 데이터입니다.
        # 컬럼: [Idx1, Idx2, Distance(Ward), Sample_Count]
        with open(os.path.join(save_dir, "Cluster_Linkage_Matrix.csv"), "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["Merge_Step", "Cluster_Idx_1", "Cluster_Idx_2", "Ward_Distance", "Member_Count"])
            for i, row in enumerate(Z):
                w.writerow([i+1] + [f"{v:.4f}" for v in row])
                
        # 4. 기준 정보 텍스트 저장
        with open(os.path.join(save_dir, "Cluster_Cut_Criteria.txt"), "w") as f:
            f.write(f"Cut Mode: {cut_mode}\n")
            f.write(f"Effective Cut Threshold (Distance): {cut_threshold:.6f}\n")
            f.write("-" * 30 + "\n")
            f.write("Note: If 'Ward_Distance' in Linkage Matrix > Threshold, those clusters are merged.\n")
            f.write("To get N clusters, the tree is cut at this distance height.\n")

        # 5. 덴드로그램 이미지 저장 (Threshold 라인 추가)
        plt.figure(figsize=(12, 6))
        dendrogram(Z, labels=valid_types, leaf_rotation=90)
        # 컷오프 라인 그리기
        if cut_threshold > 0:
            plt.axhline(y=cut_threshold, c='r', ls='--', lw=1, label=f'Cut Threshold ({cut_threshold:.2f})')
            plt.legend()
            
        plt.title(f"Hierarchical Clustering Dendrogram\n(Mode: {cut_mode}, Thresh: {cut_threshold:.2f})")
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "Type_Dendrogram.png"), dpi=150)
        plt.close()

    # -----------------------------------------------------
    # [방법 3] Cluster Ensemble Only Finder
    # -----------------------------------------------------
    def _find_cluster_ensemble(self, target_type, target_shape_res, n_clusters=4):
        """
        [Method 3] 
        1. DB를 n_clusters개로 클러스터링.
        2. 각 클러스터의 '평균(Centroid)'과 Target을 비교.
        3. 가장 가까운 클러스터의 '모든 멤버(All Members)'를 반환.
        """
        ...
        final_info = f"[Method 3] Cluster Ensemble (Group {best_cluster_id}, RMSE: {cluster_rmse:.4f}, Count: {len(best_cluster_members)})"
            
        return best_cluster_members, final_info, cluster_rmse

    # -----------------------------------------------------
    # 메인 처리 로직 (Method 3 적용)
    # -----------------------------------------------------
    def process_single_type(self, target_type, model_type, target_sizes):
        ...
        # 1. Method 3 (Cluster Ensemble) 실행
        selected_members, match_info, rmse_val = self._find_cluster_ensemble(target_type, P_new_res, n_clusters=4)
        
        # 2. 안전장치 (Threshold Check)
        # 그룹 평균과의 거리가 너무 멀면 Ratio로 전환
        THRESHOLD_RMSE = 5.0  # Method 3를 위해 기준을 넉넉하게 잡음
        
        track = []
        if rmse_val > THRESHOLD_RMSE:
            strategy_log = f"🛑 [Fallback] RMSE {rmse_val:.2f} > {THRESHOLD_RMSE} (Group mismatch)"
            matched_type = strategy_log
            track = sorted([(s, p) for s, (p, _) in target_map.items()], key=lambda x: x[0])
            print(f"    -> {strategy_log}")
            
        else:
            strategy_log = f"✅ {match_info}"
            matched_type = strategy_log
            
            # 선택된 클러스터의 '모든 멤버'를 Training Data에 추가
    ...
    # =====================================================
    # Safe Prediction Logic (Helper Methods)
    # =====================================================
