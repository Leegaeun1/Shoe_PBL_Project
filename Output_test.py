...
def row_to_xy(row: pd.Series, xs: list[str], ys: list[str]) -> np.ndarray: # Series 저건 한 행을
...
# dummy_Data_linked.py 에서 복사해올 함수
def heel_index(pts: np.ndarray, positive_down: bool) -> int:
    y = pts[:,1]
    # positive_down이 True이면 y값은 양수(스크린 좌표계), False이면 음수(수학 좌표계)
...
def closed_bspline_resample(pts: np.ndarray, n_out: int, s: float = 8.0) -> np.ndarray:
    x, y = pts[:, 0], pts[:, 1]      # x, y 좌표 분리

    # 시작점 붙여서 폐곡선으로 만듦
    x = np.r_[x, x[0]]
    y = np.r_[y, y[0]]

    # B-spline 보간 모델 생성. 스무딩한 매개변수 곡선으로 변환으로 변환. 
    # tck는 b-spline 모델 정보, u는 원래 점의 파라미터
    tck, _ = interpolate.splprep([x, y], s=s, per=True)

    # 0~1 사이에서 균일한 파라미터 n_out 개 생성
...
def heel_align(pts: np.ndarray, size_mm: float, set_x: float | None = None) -> np.ndarray:
    out = pts.copy()
    heel = out[-1]
    out[:, 1] += (0.0) - heel[1]   # 모든 행의 y값이 변함
    if set_x is not None: # 값이 들어간다면 x값도 변경됨. 
        out[:, 0] += set_x - out[-1, 0]
    return out
...
def compute_normals(poly: np.ndarray) -> np.ndarray: # 점에서의 법선 벡터 계산
    N = len(poly)
...
def ensure_heel_last(pts: np.ndarray) -> np.ndarray: # 뒤꿈치를 마지막점으로
    return rotate_last_is_index(pts, heel_index(pts,positive_down=False))

def polygon_area(P: np.ndarray) -> float: # 면적 
    Q = np.vstack([P,P[0]])
    x = Q[:,0]; y = Q[:,1]
    return 0.5*float(np.sum(x[:-1]*y[1:] - x[1:]*y[:-1])) #신발끈 공식 

def polygon_perimeter(P: np.ndarray) -> float: # 둘레
    Q = np.vstack([P,P[0]])
    return float(np.sum(np.linalg.norm(Q[1:] - Q[:-1], axis=1))) # 각 선분 길이 합

# ---------- Models ----------
def umeyama_similarity(X: np.ndarray, Y: np.ndarray, with_scaling=True):
    Xc = X.mean(axis=0); Yc = Y.mean(axis=0) # x, y 평균 
    X0 = X - Xc; Y0 = Y - Yc # 평균을 빼서 정규화시킴 
    C = (Y0.T @ X0) / X.shape[0] # 공분산 행렬. Y0.T는 (2,N), X0는 (N,2)라서 C는 (2,2)가 됨. shpae는 점의개수라 평균 공분산 구함. 
    U, S, Vt = np.linalg.svd(C) # 왼쪽 특이벡터, 특이값, 오른쪽 특이벡터의 전치
    R = U @ Vt # 회전 행렬.
    if np.linalg.det(R) < 0: # 반사를 포함하면 안됨. 행렬값이 -면 뒤집기 
        U[:, -1] *= -1
        R = U @ Vt
    if with_scaling:
        varX = (X0**2).sum() / X.shape[0] # X0 분산
        s = (S @ np.ones_like(S)) / (varX + 1e-12) # 스케일 계산
    else:
        s = 1.0 # 크기 유지
    t = Yc - s * (R @ Xc) # X를 회전, 스케일 한 뒤, 평균좌표가 Y의 평균좌표와 일치하도록.. 
    return float(s), R, t # 스케일, 회전행렬, 평행이동 벡터

def fit_pair_model(P220: np.ndarray, P270: np.ndarray) -> dict:
    s_g, R_g, t_g = umeyama_similarity(P220, P270, with_scaling=True) # 220 좌표와 270 좌표를 최대한 맞추도록 
    P220_fit = (s_g * (R_g @ P220.T)).T + t_g # 220 좌표 -> 270 좌표 근사 위치로 변환
    nrm = compute_normals(P220_fit) # 각 점에서 법선 벡터 계산
    E = P270 - P220_fit 
...
def preprocess_220_only(row_220: pd.Series, xs: list[str], ys: list[str], smooth_s=10.0) -> np.ndarray:
    ...
    # 1. 뒤꿈치 인덱스를 찾습니다. (수정된 heel_index_max_y 사용)
    heel_idx = heel_index_min_y(C)
    # 2. 뒤꿈치를 마지막 점으로 회전시킵니다.
    C = rotate_last_is_index(C, heel_idx)
    # 3. B-spline 보간 및 정렬을 수행합니다.
    C = closed_bspline_resample(C, n_out=len(xs), s=smooth_s)
    C = ensure_heel_last(C)
    C = heel_align(C, size_mm=220.0, set_x=0.0)
    return C
# ---------- Preprocessing ----------
# ... (existing functions) ...
...
def prepare_donors(df_pairs: pd.DataFrame, xs: list[str], ys: list[str], smooth_s: float) -> tuple[list[np.ndarray], list[dict]]:
    ...
    for pid, g in df_pairs.groupby("PairID"):
        ...
        # Process and align both donor shapes to a common heel
        P220 = preprocess_220_only(r220, xs, ys, smooth_s=smooth_s)
        P270 = preprocess_220_only(r270, xs, ys, smooth_s=smooth_s)
        
        # Ensure both are aligned to the same x-coordinate (already handled by the updated preprocess_220_only)
        # heel_x = P220[-1,0]
...
def choose_lambda_by_consensus(P220_t: np.ndarray, neighbors_idx: np.ndarray, w: np.ndarray,
                               donors_models: list[dict], lam_grid: list[float]) -> tuple[float, np.ndarray, float]:
...
def heel_resnap(P270: np.ndarray, P220_ref: np.ndarray) -> np.ndarray:
    ...
    return heel_align(P270, size_mm=270.0, set_x=0.0)
...
def area_resnap(P270: np.ndarray, P220_ref: np.ndarray) -> np.ndarray:
    ...
    P_adj = heel_align(P_adj, size_mm=270.0, set_x=0.0) # 이 줄을 추가합니다.
    return P_adj
