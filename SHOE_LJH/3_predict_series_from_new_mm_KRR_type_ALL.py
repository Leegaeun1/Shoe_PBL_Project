...
# -*- coding: utf-8 -*-
"""
predict_series_all_types_unified_krr.py

[기능]
1. Data_DIR 내 MASTER_CSV 로드.
2. 존재하는 모든 Type(Type00~Type07 등)을 순회.
3. 각 Type을 Hold-out(테스트용)으로 설정.
4. 나머지 Type들 중에서 P_new와 가장 유사한 Type을 선정하여 KRR 학습.
...
MASTER_CSV_NAME = "control_points_master_L_20251120.csv"

# 1. 통합 결과 저장 파일명
SAVE_FILENAME = "pred_Data_KRR_230_280.csv"
SAVE_PRED_PATH = os.path.join(Data_DIR, SAVE_FILENAME)

# 2. 실행 시간 요약 저장 경로 (★ 추가된 부분)
SAVE_SUMMARY_FILENAME = "krr_runtime_summary.csv"
...
def krr_fit_predict_safe(x_train, Y, x_test, alpha=1e-2, length_scale=20.0, 
                         linear_weight=1.0, rbf_weight=1.0, tail_k_local=3, tail_tau_mm=8.0):
    ...
    def kernel_callable(A, B):
        ...
        K = 0.0
        if rbf_weight != 0.0: K += float(rbf_weight) * rbf_kernel(A, B, gamma=gamma)
        if linear_weight != 0.0: K += float(linear_weight) * linear_kernel(A, B)
        return K
    ...
    aL, bL = _linear_fit_multi(x_train[:tail_k_local], Y[:tail_k_local])
    aR, bR = _linear_fit_multi(x_train[-tail_k_local:], Y[-tail_k_local:])
...
def process_single_type(target_type, full_data):
    ...
    try:
        dt_pred = krr_fit_predict_safe(sizes_train, Ydt, TARGET_SIZES.astype(float))
        dn_pred = krr_fit_predict_safe(sizes_train, Ydn, TARGET_SIZES.astype(float), tail_tau_mm=0.1)
    except Exception as e:
        print(f"  [Error] KRR fit failed: {e}")
        return [], "KRR Fail", 0
