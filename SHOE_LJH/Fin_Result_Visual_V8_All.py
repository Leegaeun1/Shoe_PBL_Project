...
MASTER_FILENAME = "control_points_master_R_20251126.csv"
...
STYLE_RULES = {
    ...
    # 단일 모델
    "GPR":      {"color": "green",     "ls": "-",   "lw": 1.5},      # 초록
    "SVR":      {"color": "purple",    "ls": "-.",  "lw": 1.5},      # 보라
    "KRR":      {"color": "blue",      "ls": "--",  "lw": 1.5},      # 파랑
    "PCA":      {"color": "red",       "ls": ":",   "lw": 2.0},      # 빨강
...
def get_metrics(C_ref, C_pred):
    ...
    # 6. Geometric Distance (Chamfer/Hausdorff)
    ...
    return {
        ...
        "Length_Error": len_err, "Width_Error": w_err, "Size_Error": size_err, "Area_Error": area_err,
            ...
            for size in target_sizes:
                ...
                for m_name, db in model_dbs.items():
                    if t_type in db and size in db[t_type]:
                        ...
                        # [수정] RATIO_CTRL 특수 처리: 해당 Type의 첫 번째(최소) 사이즈 제외
                        if "RATIO_CTRL" in m_name.upper():
                            available_sizes = sorted(db[t_type].keys())
                            if available_sizes and size == available_sizes[0]:
                                # print(f"  [Skip] RATIO_CTRL {t_type} first size {size}")
                                continue
