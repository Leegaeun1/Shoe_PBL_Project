...
#OUTPUT_DIR = f"Fin_Excel_Data_{currentTime}" 
OUTPUT_DIR = f"control_points_master_test_Q" 
...
MASTER_CSV = os.path.join(
    OUTPUT_DIR,
    f"control_points_master_test_Q.csv"
)
    ...
    try:
        ...
        if align_to_heel and data_plots:
            ...
            if ref_data is not None:
                try:
                    ...
                    print(f"[WARN] Alignment failed for {title}: {e}. Disabling alignment.")
                    align_to_heel = False
            else:
                align_to_heel = False
        ...
        for item in data_plots:
            ...
            if points.ndim == 2 and points.shape[1] == 2 and len(points) > 0:
                ...
                print(f"[WARN] Invalid data shape {points.shape if points is not None else 'None'} for plot, skipping.")

        if not all_data_for_bounds:
            print(f"[WARN] No valid data to plot for {title}, skipping save.")
            plt.close(fig)
            return
...
def pca_major_axis(P):
    ...
    try:
        ...
        # SVD 실패 시 (모든 점이 한 점에 겹친 경우 등) 임의 축 반환
...
def run_open_fit_DIR(DIR_path, n_contour_points=200, n_ctrl_points=25, degree=3):
    ...
    os.makedirs(dir_1_mask, exist_ok=True)
    os.makedirs(dir_2_contour, exist_ok=True)
    os.makedirs(dir_3_polygon, exist_ok=True)
    os.makedirs(dir_4_bspline, exist_ok=True)
    ...
    for img in jpgs:
        try:
            ...
            save_path_mask = os.path.join(dir_1_mask, filename_base + ".png")
            try:
            ...
            save_path_contour = os.path.join(dir_2_contour, filename_base + ".png")
            save_plot(save_path_contour, 
                      title=f"2. Pixel Contour (Raw)\n{filename_base}", 
                      data_plots=[
                          {'data': contour, 'style': 'b.', 'label': 'Raw Contour Pts'}
                      ],
                      align_to_heel=False) 
            ...
            # --- 4. mm 스케일 변환 (mm단위 다각형) ---
            size_mm = parse_size_mm_from_filename(img, fallback=250.0)
            contour_mm = scale_to_mm(contour_path, height_mm=size_mm, ref_axis="y")
            
            save_path_polygon = os.path.join(dir_3_polygon, filename_base + ".png")
            save_plot(save_path_polygon,
                      title=f"3. Resampled Polygon (mm-space)\n{filename_base}",
                      data_plots=[
                          {'data': contour_mm, 'style': 'k.-', 'label': f'Polygon ({n_contour_points} pts)'}
                      ],
                      align_to_heel=True) 

            # --- 5. B-스플라인 피팅 (mm단위 제어점) ---
            ctrl, knots = fit_open_bspline_least_squares(
                contour_mm, n_ctrl=n_ctrl_points, degree=degree
            )

            # --- 6. 최종 B-스플라인 곡선 생성 및 저장 ---
            ...
            save_plot(save_path_bspline,
                      title=f"4. B-Spline Fit (mm-space)\n{filename_base}",
                      data_plots=[
                          {'data': contour_mm, 'style': 'k-', 'label': 'Polygon (Target)', 'zorder': 1},
                          {'data': final_curve, 'style': 'r-', 'label': 'B-Spline Curve (Closed)', 'zorder': 3}, 
                          {'data': ctrl, 'style': 'bo', 'label': f'Control Points ({n_ctrl_points})', 'zorder': 4},
                          {'data': ctrl_closed, 'style': 'b--', 'label': 'Control Polygon (Closed)', 'zorder': 2} # ★ 수정
                      ],
                      align_to_heel=True) 
...
if __name__ == "__main__":
    ...
    DIR_PATH = r"C:\Users\user\Documents\GitHub\Shoe_PBL_Project\SHOE_LJH\output_outlines_1030\test_Set"  # <- 여기에 대상 디렉토리 경로 입력
